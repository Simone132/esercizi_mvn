    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                // la variabile prezzo in virgola viene creata, per permettere la
                // visualizzazione del dato del prezzo con la virgola anziche che con il punto
                String prezzoInVirgola = Double.toString(rs.getDouble("prezzo")).replace(".", ",");
                if (rs.getString("nome").length() > 20) {
                    if (prezzoInVirgola.length() > 6) {
                        System.out.println(
